...
public class DatabaseConnections
{
    ...
    public SetupInformation SetupInfo => new SetupInformation()
    {
        ...
        SecretSource = _configuration.GetValue<string>("ApplicationSecrets:SecretStorage") switch
        {
            "AzureKeyVault" => $"Azure: {Environment.GetEnvironmentVariable("AZURE_KeyVaultSecret")}",
            _ => $"Usersecret: {Environment.GetEnvironmentVariable("USERSECRETID")}"
        },
